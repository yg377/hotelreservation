...
class WP_Launcher {
	...
	public function __construct() {
		$default_settings = array(
			...
			'subscribe' => array(
				...
				'include_name' => 0,
				'include_last_name' => 0,
				'submit_label' => __('Sign Up', 'launcher'),
				'name_label' => __('First Name', 'launcher'),
				'last_name_label' => __('Last Name', 'launcher'),
				'email_label' => __('Email', 'launcher'),
				'success_message' => __('Thanks for signing up!', 'launcher'),
				'feedburner' => array(
					'username' => ''
				),
				'mailchimp' => array(
			...
			'contact' => array(
				'label_name' => __('Name', 'launcher'),
				'label_email' => __('Email', 'launcher'),
				'label_message' => __('Message', 'launcher'),
				'label_submit' => __('Send Message', 'launcher'),
		...
		if (is_admin()) {
			...
			// Frontend-only stuff
			add_filter( 'template_include', array( $this, 'launch' ), 9999 );
	...
	function admin_menu() {
	    add_options_page(__('Launcher', 'launcher'), __('Launcher', 'launcher'), 'manage_options', 'launcher', array( $this, 'settings_page' ) );
	}
	...
	function settings_page() {
	    if ( ! current_user_can( 'manage_options' ) ) {
	        wp_die( __( 'You do not have sufficient permissions to access this page.', 'launcher' ) );
	    }

	    if ( ! defined( 'LAUNCHER_DISABLE_SCRIPT_INPUT' ) ) {
			define( 'LAUNCHER_DISABLE_SCRIPT_INPUT', false );
		}
	    ?>
	    <div class="wrap">
	    	...
	    	<form method="post" action="options.php">
                ...
                <h3 class="nav-tab-wrapper wplauncher-nav-tab-wrapper">
			    	<a href="#" class="nav-tab nav-tab-active" id="wplauncher-tab-general" data-rel="#wplauncher-settings-general"><?php _e('General', 'launcher'); ?></a>
			    	<a href="#" class="nav-tab" id="wplauncher-tab-templates" data-rel="#wplauncher-settings-templates"><?php _e('Templates', 'launcher'); ?></a>
			    	<a href="#" class="nav-tab<?php echo (!$this->template_supports( 'countdown' ) ? ' wplauncher-not-supported' : ''); ?>" id="wplauncher-tab-countdown" data-rel="#wplauncher-settings-countdown"><?php _e('Countdown Timer', 'launcher'); ?></a>
			    	<a href="#" class="nav-tab<?php echo (!$this->template_supports( 'subscribe' ) ? ' wplauncher-not-supported' : ''); ?>" id="wplauncher-tab-subscribe" data-rel="#wplauncher-settings-subscribe"><?php _e('Subscribe Form', 'launcher'); ?></a>
			    	<a href="#" class="nav-tab<?php echo (!$this->template_supports( 'twitter' ) ? ' wplauncher-not-supported' : ''); ?>" id="wplauncher-tab-twitter" data-rel="#wplauncher-settings-twitter"><?php _e('Twitter Feed', 'launcher'); ?></a>
			    	<a href="#" class="nav-tab<?php echo (!$this->template_supports( 'contact' ) ? ' wplauncher-not-supported' : ''); ?>" id="wplauncher-tab-contact" data-rel="#wplauncher-settings-contact"><?php _e('Contact Form', 'launcher'); ?></a>
			    	<a href="#" class="nav-tab<?php echo (!$this->template_supports( 'social' ) ? ' wplauncher-not-supported' : ''); ?>" id="wplauncher-tab-social" data-rel="#wplauncher-settings-social"><?php _e('Social Links', 'launcher'); ?></a>
	    		</h3>
	    		<div class="wplauncher-nav-tab-contents">
	    			<div id="wplauncher-settings-general">
	    				<table class="form-table">
		                    <?php $this->options_field_checkbox('wplauncher_options[enabled]', __('Enable Launcher Page', 'launcher'), __('If enabled, only administrators can access the frontend of the site.', 'launcher'));

		                    $this->options_field_text('wplauncher_options[page_title]', __('Page Title', 'launcher'));

		                    $this->options_field_text('wplauncher_options[favicon]', __('Favicon', 'launcher'), '', array('class' => 'large-text'));
							?>
		                    ...
		                    <?php
		                    $this->options_field_text('wplauncher_options[meta_description]', __('Meta Description', 'launcher'), __('Leave this field empty to omit meta description tag.', 'launcher'), array('class' => 'large-text'));
		                    $this->options_field_checkbox('wplauncher_options[noindex]', __('<em>Noindex</em> Meta', 'launcher'), __('If enabled, search engines won&rsquo;t index the launcher page, regardless of your robots.txt settings.', 'launcher'));
		                    ...
		                    $this->options_field_select('wplauncher_options[admin_role]', $roles, __('Access Role', 'launcher'), __('Select user roles who will see the real site instead of the launcher page.', 'launcher'), array('multiple' => 'multiple'));*/

		                    //$this->options_field_select('wplauncher_options[user_access]', array('administrator' => __('Administrator', 'launcher'), 'loggedin' => __('All logged-in users', 'launcher')), __('Access Role', 'launcher'), __('Choose whether only administrators will see the real site instead of the launcher page, or any logged in user as well.', 'launcher'));

		                    $this->options_field_textarea('wplauncher_options[custom_css]', __('Custom CSS Code', 'launcher'), __('Additional CSS code to adjust styling.', 'launcher'), array('class' => 'large-text'));
		                    if ( ! LAUNCHER_DISABLE_SCRIPT_INPUT ) {
		                    	$this->options_field_textarea('wplauncher_options[header_code]', __('Header Code', 'launcher'), __('Additional code inserted in the <code>&lt;head&gt;</code> area of the page.', 'launcher'), array('class' => 'large-text'));
		                    	$this->options_field_textarea('wplauncher_options[footer_code]', __('Footer Code', 'launcher'), __('Additional code inserted before the closing <code>&lt;/body&gt;</code> tag.', 'launcher'), array('class' => 'large-text'));
		                    }
	    			...
	    			<div id="wplauncher-settings-templates" style="display: none;">
	    				<p><?php echo sprintf(__('You can access the <a href="%s">template editor</a> from the admin bar at the top.', 'launcher'), add_query_arg('wplauncher_edit_template', '1', get_bloginfo( 'url' ))); ?></p>
	    			...
	    			<div id="wplauncher-settings-countdown" style="display: none;">
	    				<div class="wplauncher-error"<?php echo ($this->template_supports( 'countdown' ) ? ' style="display: none"' : ''); ?>>
	    					<p><?php _e('The currently selected template does not declare support for the Countdown Timer. It is possible that this element won\'t show up on the launcher page.', 'launcher'); ?></p>
	    				</div>

	    				<table class="form-table">
	    					<?php
		                    $this->options_field_text('wplauncher_options[countdown][date_formatted]', __('Countdown To', 'launcher'));
		                    $this->options_field_checkbox('wplauncher_options[countdown][schedule_disable]', __('Disable Launcher Page', 'launcher'), __('Disables launcher mode at the specified date/time (if the countdown timer is present on your launcher page).', 'launcher'));
		                	?>
		                </table>
	    			...
	    			<div id="wplauncher-settings-subscribe" style="display: none;">
	    				<div class="wplauncher-error"<?php echo ($this->template_supports( 'subscribe' ) ? ' style="display: none"' : ''); ?>>
	    					<p><?php _e('The currently selected template does not declare support for the Subscribe Form. It is possible that this element won\'t show up on the launcher page.', 'launcher'); ?></p>
	    				</div>

	    				<table class="form-table">
		                    <?php
		                    	$this->options_field_select('wplauncher_options[subscribe][service]', array(
		                    		'feedburner' => __( 'Feedburner', 'launcher' ),
									'mailchimp' => __( 'MailChimp', 'launcher' ),
									'aweber' => __( 'Aweber', 'launcher' ),
									'getresponse' => __( 'Get Response', 'launcher' ),
									'campaignmonitor' => __( 'Campaign Monitor', 'launcher' ),
									'madmimi' => __( 'Mad Mimi', 'launcher' ),
								), __('Service', 'launcher'));

		                    	// Mailchimp
		                    	$this->options_field_text('wplauncher_options[subscribe][mailchimp][api_key]', __('Mailchimp <a href="http://kb.mailchimp.com/accounts/management/about-api-keys" target="_blank">API key</a>', 'launcher'), '', array(), 'show-if-subscribe if-mailchimp');
		                    	$this->options_field_select('wplauncher_options[subscribe][mailchimp][list]', array(), __('List', 'launcher'), '', array(), 'show-if-subscribe if-mailchimp');

		                    	// Aweber
		                    	?>
		                    	<tr class="show-if-subscribe if-aweber">
						            <th><?php _e('Connect with AWeber', 'launcher') ?></th>
						            <td>
						            	<a href="https://auth.aweber.com/1.0/oauth/authorize_app/a3f7ff6f" target="_blank" class="button wplauncher-aweber-connect"><?php isset($this->settings['subscribe']['aweber']['access_key']) && $this->settings['subscribe']['aweber']['access_key'] == '' ? _e( 'Get Authorization Code', 'launcher' ) : _e( 'Reconnect Account', 'launcher' ); ?></a>
								...
								<?php
		                    	$this->options_field_text('wplauncher_options[subscribe][aweber][code]', __('Paste authorization code', 'launcher'), '', array(), 'show-if-subscribe if-aweber');
		                    	$this->options_field_select('wplauncher_options[subscribe][aweber][list]', array(), __('List', 'launcher'), '', array(), 'show-if-subscribe if-aweber');

		                    	// GetResponse
		                    	$this->options_field_text('wplauncher_options[subscribe][getresponse][api_key]', __('GetResponse <a href="http://www.getresponse.com/learning-center/glossary/api-key.html" target="_blank">API key</a>', 'launcher'), '', array(), 'show-if-subscribe if-getresponse');
		                    	$this->options_field_select('wplauncher_options[subscribe][getresponse][campaign]', array(), __('Campaign', 'launcher'), '', array(), 'show-if-subscribe if-getresponse');

		                    	// Campaign Monitor
		                    	$this->options_field_text('wplauncher_options[subscribe][campaignmonitor][api_key]', __('Campaign Monitor <a href="http://help.campaignmonitor.com/topic.aspx?t=206" target="_blank">API key</a>', 'launcher'), '', array(), 'show-if-subscribe if-campaignmonitor');
		                    	$this->options_field_select('wplauncher_options[subscribe][campaignmonitor][client]', array(), __('Client', 'launcher'), '', array(), 'show-if-subscribe if-campaignmonitor');
		                    	$this->options_field_select('wplauncher_options[subscribe][campaignmonitor][list]', array(), __('List', 'launcher'), '', array(), 'show-if-subscribe if-campaignmonitor');

		                    	// Mad Mimi
		                    	$this->options_field_text('wplauncher_options[subscribe][madmimi][username]', __('Mad Mimi username/email', 'launcher'), '', array(), 'show-if-subscribe if-madmimi');
		                    	$this->options_field_text('wplauncher_options[subscribe][madmimi][api_key]', __('Mad Mimi <a href="http://help.madmimi.com/where-can-i-find-my-api-key/" target="_blank">API key</a>', 'launcher'), '', array(), 'show-if-subscribe if-madmimi');
		                    	$this->options_field_select('wplauncher_options[subscribe][madmimi][list]', array(), __('List', 'launcher'), '', array(), 'show-if-subscribe if-madmimi');

		                    	// Feedburner
		                    	$this->options_field_text('wplauncher_options[subscribe][feedburner][username]', __('Feedburner username', 'launcher'), '', array(), 'show-if-subscribe if-feedburner');


		                    	$this->options_field_checkbox('wplauncher_options[subscribe][include_name]', __('Include name field', 'launcher'), '', array(), 'show-if-subscribe if-mailchimp if-aweber if-getresponse if-campaignmonitor if-madmimi');
		                    	$this->options_field_checkbox('wplauncher_options[subscribe][include_last_name]', __('Include last name field', 'launcher'), '', array(), 'show-if-subscribe if-mailchimp if-aweber if-getresponse if-campaignmonitor if-madmimi');

		                    	$this->options_field_text('wplauncher_options[subscribe][name_label]', __('Name field label', 'launcher'));
		                    	$this->options_field_text('wplauncher_options[subscribe][last_name_label]', __('Last name field label', 'launcher'));
		                    	$this->options_field_text('wplauncher_options[subscribe][email_label]', __('Email field label', 'launcher'));
		                    	$this->options_field_text('wplauncher_options[subscribe][submit_label]', __('Submit button label', 'launcher'));
		                    	$this->options_field_text('wplauncher_options[subscribe][success_message]', __('Success message', 'launcher'));
		                    ?>
		                </table>
	    			</div>
	    			...
	    			<div id="wplauncher-settings-twitter" style="display: none;">
	    				<div class="wplauncher-error"<?php echo ($this->template_supports( 'twitter' ) ? ' style="display: none"' : ''); ?>>
	    					<p><?php _e('The currently selected template does not declare support for the Twitter Feed. It is possible that this element won\'t show up on the launcher page.', 'launcher'); ?></p>
	    				</div>

	    				<table class="form-table">
	    					<p><?php _e('You can generate your API Key from here:', 'launcher'); ?> <a href="https://apps.twitter.com/">https://apps.twitter.com/</a></p>
	    					<?php
		                    $this->options_field_text('wplauncher_options[twitter][username]', __('Twitter Username', 'launcher'));
		                    $this->options_field_text('wplauncher_options[twitter][api_key]', __('API Key', 'launcher'));
		                    $this->options_field_text('wplauncher_options[twitter][api_secret]', __('API Secret', 'launcher'));
		                    $this->options_field_text('wplauncher_options[twitter][access_token]', __('Access Token', 'launcher'));
		                    $this->options_field_text('wplauncher_options[twitter][access_token_secret]', __('Access Token Secret', 'launcher'));
		                    $this->options_field_text('wplauncher_options[twitter][cache_time]', __('Tweets Caching Time', 'launcher'), __('Refresh tweets every X hours', 'launcher'), array('type' => 'number', 'class' => 'small-text'));
		                	?>
		                </table>
	    			</div>
	    			...
	    			<div id="wplauncher-settings-contact" style="display: none;">
	    				<div class="wplauncher-error"<?php echo ($this->template_supports( 'contact' ) ? ' style="display: none"' : ''); ?>>
	    					<p><?php _e('The currently selected template does not declare support for the Contact Form. It is possible that this element won\'t show up on the launcher page.', 'launcher'); ?></p>
	    				</div>

	    				<table class="form-table">
	    					<?php
		                    $this->options_field_text('wplauncher_options[contact][sendto]', __('Send emails to', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[contact][label_name]', __('Name field label', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[contact][label_email]', __('Email field label', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[contact][label_message]', __('Message field label', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[contact][label_submit]', __('Submit button label', 'launcher'), '', array('class' => 'large-text'));
		                	?>
		                </table>
	    			</div>
	    			...
	    			<div id="wplauncher-settings-social" style="display: none;">
	    				<div class="wplauncher-error"<?php echo ($this->template_supports( 'social' ) ? ' style="display: none"' : ''); ?>>
	    					<p><?php _e('The currently selected template does not declare support for the Social Links. It is possible that this element won\'t show up on the launcher page.', 'launcher'); ?></p>
	    				</div>

	    				<table class="form-table">
		                    <?php
		                    $this->options_field_text('wplauncher_options[social][facebook]', __('Facebook Page URL', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[social][twitter]', __('Twitter Page URL', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[social][instagram]', __('Instagram Page URL', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[social][youtube]', __('Youtube Page URL', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[social][linkedin]', __('LinkedIn Page URL', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[social][googleplus]', __('Google+ Page URL', 'launcher'), '', array('class' => 'large-text'));
		                    $this->options_field_text('wplauncher_options[social][rss]', __('RSS URL', 'launcher'), '', array('class' => 'large-text'));
		                    ?>
		                </table>
	    			</div>
	    		</div>
	...
	public function template_footer() {
		...
		if ( ! empty( $this->settings['custom_css'] ) ) {
			echo '<style type="text/css">' . strip_tags( $this->settings['custom_css'] ) . '</style>';
		}

		// footer code
		if ( ! empty( $this->settings['footer_code'] ) && ! LAUNCHER_DISABLE_SCRIPT_INPUT )
		...
		// color picker
		echo '<script>var wpColorPickerL10n = {"clear":"'.__('Clear', 'launcher').'","defaultString":"Default","pick":"Select Color","current":"Current Color"};</script>';
	...
	public function template_head() {
		...
		$output .= '<title>' . esc_html( $this->settings['page_title'] ) . '</title>';
		if (!empty($this->settings['favicon']))
			$output .= '<link rel="shortcut icon" href="'.esc_attr( $this->settings['favicon'] ).'" />';

		if (!empty($this->settings['meta_description']))
			$output .= '<meta name="description" content="'.esc_attr( $this->settings['meta_description'] ).'" />';
		...
		$output .= '<script>var ajaxurl = "'.admin_url( 'admin-ajax.php' ).'", ajax_save_url = "'.add_query_arg( array( 'action' => 'wplauncher_save_template_data', 'launcher_nonce' => wp_create_nonce( 'launcher_save_template_data' ) ), admin_url( 'admin-ajax.php' )).'", wplauncher_template = "'.$this->settings['template'].'";</script>';

		if ($this->is_editor())
			$output .= '<script src="'.WPLAUNCHER_URI.'js/jquery.jeditable.min.js"></script>';

		if ( ! empty( $this->settings['header_code'] ) && ! LAUNCHER_DISABLE_SCRIPT_INPUT )
	...
	public function ajax_save_template_data() {
		check_ajax_referer( 'launcher_save_template_data', 'launcher_nonce' );
		if ( ! current_user_can('manage_options') ) {
			exit('0');
		}
	...
	public function template_tag_social( $params ) {
		...
		if (!$is_set_up) {
			if ($this->is_editor())
				echo '<span class="wplauncher-configure wplauncher-configure-social">'.sprintf(__('To use the Social Links, insert at least one URL on the Social tab of the <a href="%s">Settings page</a>.', 'launcher'), admin_url('options-general.php?page=launcher#social')).'</span>';
			return;
		}
		// default single string param
		...
		foreach ($this->settings['social'] as $site => $url) {
			if (!empty($url)) {
				if (is_array($params['hide_links']) && in_array($site, $params['hide_links'])) {
					...
					// do
					$output .= str_replace( array( '%url%', '%icon%', '%name%' ), array( esc_attr( $url ), '<i class="wplauncher-icon-'.$site.'"></i>', $name_index[$site] ), $params['format'] );
					$output_arr[$site] = $url;
				}
			}
		}
	...
	public function template_tag_countdown( $params = '' ) {
		if (empty($this->settings['countdown']['date_formatted'])) {
			...
			if ($this->is_editor())
				echo '<span class="wplauncher-configure wplauncher-configure-countdown">'.sprintf(__('To use the Countdown Timer, set it up on the <a href="%s">Settings page</a>.', 'launcher'), admin_url('options-general.php?page=launcher#countdown')).'</span>';
			return;
		} elseif ($this->settings['countdown']['date'] < time()) {
			// date expired
			if ($this->is_editor())
				echo '<span class="wplauncher-configure wplauncher-configure-countdown">'.sprintf(__('To use the Countdown Timer, set it to a date in the future on the <a href="%s">Settings page</a>.', 'launcher'), admin_url('options-general.php?page=launcher#countdown')).'</span>';
			return;
		}
		...
		$params = wp_parse_args( $params, array(
			'format' => '
				<span class="wplauncher-days">%D</span>
					 <span class="wplauncher-date-sep">'.__('days', 'launcher').'</span>
	...
	public function template_tag_twitter( $params ) {
		...
		if(empty($this->settings['twitter']['api_key']) || empty($this->settings['twitter']['api_secret']) || empty($this->settings['twitter']['access_token']) || empty($this->settings['twitter']['access_token_secret']) || empty($this->settings['twitter']['username'])) {
			if ($this->is_editor())
				echo '<span class="wplauncher-configure wplauncher-configure-twitter">'.sprintf(__('To display the Twitter feed, set it up on the <a href="%s">Settings page</a>.', 'launcher'), admin_url('options-general.php?page=launcher#twitter')).'</span>';
			return;
		}
		...
		if($diff >= $crt || empty($mts_twitter_plugin_last_cache_time)){
			...
			if (empty($tweets)) {
				if ($this->is_editor())
					echo '<span class="wplauncher-configure wplauncher-configure-twitter">'.sprintf(__('Couldn\'t retreive feed. Make sure you have set the username correctly on the <a href="%s">Settings page</a>.', 'launcher'), admin_url('options-general.php?page=launcher#twitter')).'</span>';
				return;
			}
	...
	public function twitter_relative_time($a) {
		...
		if(is_numeric($d) && $d > 0) {
			...
			if($d < $minute) return sprintf(__('%s seconds ago', 'launcher'), floor($d)); //if less then minute
			if($d < $minute * 2) return __('about 1 minute ago', 'launcher'); //if less then 2 minutes
			if($d < $hour) return sprintf(__('%s minutes ago', 'launcher'), floor($d / $minute)); //if less then hour
			if($d < $hour * 2) return __('about 1 hour ago', 'launcher'); //if less then 2 hours
			if($d < $day) return sprintf(__('%s hours ago', 'launcher'), floor($d / $hour)); //if less then day
			if($d > $day && $d < $day * 2) return __('yesterday', 'launcher'); //if more then day, but less then 2 days
			if($d < $day * 365) return sprintf(__('%s days ago', 'launcher'), floor($d / $day)); //if less then year
			return __('over a year ago', 'launcher'); //else return more than a year
		}
	}
	...
	public function template_tag_contact( $params ) {
		if(empty($this->settings['contact']['label_name']) || empty($this->settings['contact']['label_email']) || empty($this->settings['contact']['label_message']) || empty($this->settings['contact']['sendto'])) {
			if ($this->is_editor())
				echo '<span class="wplauncher-configure wplauncher-configure-contact">'.sprintf(__('To display the Contact Form, set it up on the <a href="%s">Settings page</a>.', 'launcher'), admin_url('options-general.php?page=launcher#contact')).'</span>';
			return;
		}
		$params = wp_parse_args( $params, array(
			'hideable' => 1,
	...
	public function template_tag_subscribe( $params ) {
		if(empty($this->settings['subscribe']['service'])) {
			if ($this->is_editor())
				echo '<span class="wplauncher-configure wplauncher-configure-subscribe">'.sprintf(__('To display the Subscribe Form, set it up on the <a href="%s">Settings page</a>.', 'launcher'), admin_url('options-general.php?page=launcher#subscribe')).'</span>';
			return;
		}
			...
			<form id="wplauncher-subscribe" method="post" novalidate="true">
				<fieldset class="wplauncher-subscribe-fieldset">
					<?php if ( $this->settings['subscribe']['include_name'] ) { ?>
						<input id="wplauncher-subscribe-first-name" name="wplauncher-subscribe-fname" type="text" placeholder="<?php echo esc_attr( $this->settings['subscribe']['name_label'] ); ?>"/>
					<?php } ?>

					<?php if ( $this->settings['subscribe']['include_last_name'] ) { ?>
						<input id="wplauncher-subscribe-last-name" name="wplauncher-subscribe-lname" type="text" placeholder="<?php echo esc_attr( $this->settings['subscribe']['last_name_label'] ); ?>"/>
					<?php } ?>
					<input id="wplauncher-subscribe-email" name="wplauncher-subscribe-email" type="email" placeholder="<?php echo esc_attr( $this->settings['subscribe']['email_label'] ); ?>"/>
					<input value="<?php echo esc_attr( $this->settings['subscribe']['submit_label'] ); ?>" type="submit" id="wplauncher-subscribe-submit" name="wplauncher-submit" />
	...
	public function options_field_text($name, $label, $description = '', $attrs = false, $row_class = '') {
		...
		<tr class="<?php echo $attrs['id'].' '.$row_class; ?>">
            <th><label for="<?php echo $attrs['id']; ?>"><?php echo wp_filter_post_kses( $label ); ?></label></th>
            <td>
            	<input<?php echo $attributes; ?>>
            	<?php if (!empty($description)) { ?>
                	<p class="description"><?php echo wp_filter_post_kses( $description ); ?></p>
                <?php } ?>
            </td>
        </tr>
		<?php
            ...
            <td>
            	<select<?php echo $attributes; ?>>
            		...
            		foreach ($options as $option_val => $option_label) {
            			echo '<option value="'.esc_attr( $option_val ).'"'.selected($option_val, $selected, false).'>'.$option_label.'</option>';
            		}
            		?>
            	</select>
            	<?php if (!empty($description)) { ?>
